...
export async function generateSitemaps() {
  try {
    const response = await carModelService.list({}, { take: 1 });
...
export default async function sitemap({
  ...
  try {
    ...
    const response = await carModelService.list(
      {},
      { take: SITEMAP_SIZE, skip: skip },
    );

    return safeMap<any, MetadataRoute.Sitemap[number]>(
      response?.items,
      (article) => ({
        url: `${domain}/dong-xe/${article.slug}`,
