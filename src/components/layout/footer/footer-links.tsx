...
import { Brand, Category, PostCategory } from "@/types";
...
export function FooterLinks({
  ...
  console.log("FOOOTER LINKS RENDER => ", { categories, brands });
  const sections: FooterLinkSection[] = useMemo(() => {
    const categorySection: FooterLinkSection = {
      id: "category",
    ...
    const brandSection: FooterLinkSection = {
      id: "brand",
    ...
    const postCategorySection: FooterLinkSection = {
      id: "postCategory",
      title: "Danh mục bài viết",
      links: postcategories.map((postCategory) => ({
        label: postCategory.name,
        href: `/blog/${postCategory.slug}`,
      })),
    };

    return [categorySection, brandSection, postCategorySection];
  }, [categories, brands, postcategories]);
...
function FooterMobileSection({ section }: FooterSectionProps) {
  return (
    <AccordionItem value={section.id} className="border-b border-border/50">
      <AccordionTrigger className="group py-4 text-left hover:no-underline [&>svg]:text-white **:data-[slot=accordion-trigger-icon]:text-white">
        <span className="text-sm font-semibold uppercase tracking-wide text-button-danger-foreground">
          {section.title}
        </span>
...
function FooterLinkItem({ label, href, external }: FooterLinkItemProps) {
  return (
    ...
    >
      <span className="relative hover:text-primary">
        {label}
